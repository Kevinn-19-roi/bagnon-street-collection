      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 16 }}>
              ...
              {categories.map((cat, i) => (
                <tr key={cat.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)' }}>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <Badge label={cat.active ? 'Active' : 'Inactive'} variant={cat.active ? 'success' : 'default'} />
                  </td>
                  <td style={{ padding: '12px 16px' }}>
                    <form action={deleteCategory.bind(null, cat.id)} onSubmit={e => { if (!confirm(`Supprimer "${cat.name}" ?`)) e.preventDefault() }}>
        ...
        <div style={{ background: '#17171B', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 6, padding: 20, height: 'fit-content' }}>
          ...
          <form action={createCategory} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div>
              <label style={{ display: 'block', fontFamily: 'var(--font-display)', fontSize: 10, fontWeight: 700, letterSpacing: '.15em', textTransform: 'uppercase', color: '#94938E', marginBottom: 6 }}>Ordre d'affichage</label>
